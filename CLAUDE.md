...
- Each version bump also adds an entry to the top of `CHANGELOG_RECENT` in `client/src/changelog.ts` — this feeds the in-app "What's changed" popup shown after an update. Keep it short (a sentence or two, shorter than the `CHANGELOG.md` entry), tag it `added`/`changed`/`fixed`/`removed`, and follow the same language rules (no em-dashes, concise). Skip it only for changes with nothing user-visible to say (pure refactors, internal fixes). When `CHANGELOG_RECENT` grows past about fifteen entries, move the oldest ones to the top of `client/src/changelogArchive.ts` so they stay out of the bundle everyone downloads.
- Commit and push only when asked, and when asked, do it on `main`. **Do not create a feature branch.** The user is usually connected to this session remotely, and pushing to `main` is how the work gets deployed and tested (see the Deploy section and the `/commit-push` skill). A branch parks finished work somewhere the deploy cannot see it.
